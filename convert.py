...
def load_torch_weights_and_config(
    ...
    if name_or_path.endswith(".pt"):
        ...
        name_or_path_obj = Path(name_or_path)
        pt_path = name_or_path_obj / "pytorch_model.bin"
        if pt_path.is_file():
            weights = torch.load(pt_path, map_location="cpu")
        elif list(name_or_path_obj.glob("model-*.safetensors")):
            from safetensors.numpy import load_file
            weights = {}
            for chunk in name_or_path_obj.glob("model-*.safetensors"):
                weights.update(load_file(chunk))
        elif list(name_or_path_obj.glob("pytorch_model-*.bin")):
            weights = {}
            for chunk in name_or_path_obj.glob("pytorch_model-*.bin"):
                weights.update(torch.load(chunk, map_location="cpu"))
        else:
            weights = mx.load(str(name_or_path_obj / "model.safetensors"))
        with open(name_or_path_obj / "config.json", "r") as fp:
            config = json.load(fp)
...
def convert(name_or_path: str, dtype: mx.Dtype = mx.float16):
    def remap(key, value):
        ...
        if isinstance(value, torch.Tensor):
            value = mx.array(value.detach().numpy())
        else:
